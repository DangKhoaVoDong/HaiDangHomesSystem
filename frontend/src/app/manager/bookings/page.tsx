...
const statuses: Record<number,{label:string,style:string}> = {
  1:{label:'Chờ xử lý',style:'bg-amber-100 text-amber-700'},
  2:{label:'Đã xác nhận',style:'bg-green-100 text-green-700'},
  5:{label:'Đã từ chối',style:'bg-red-100 text-red-700'},
};
...
export function BookingManagementView({adminMode=false}:{adminMode?:boolean}) {
  ...
  return <main className="min-h-screen bg-[#faf8f7] p-4 sm:p-7 lg:p-10">
    ...
    <div className="mt-7 flex gap-2 overflow-x-auto border-b">{[{v:null,l:'Tất cả'},{v:1,l:'Chờ xử lý'},{v:2,l:'Đã xác nhận'},{v:5,l:'Đã từ chối'}].map(t=><button key={String(t.v)} onClick={()=>setTab(t.v)} className={`shrink-0 border-b-2 px-5 py-3 ${tab===t.v?'border-orange-600 font-semibold text-orange-600':'border-transparent'}`}>{t.l}</button>)}</div>
