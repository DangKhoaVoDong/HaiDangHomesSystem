...
export default function PropertyDetailPage() {
  ...
  return <main className="min-h-screen bg-[#f7f7f7] text-[#171717]">
    ...
    {roomModal && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-0 sm:p-5" onClick={() => setRoomModal(null)}><div className="relative grid h-full w-full max-w-6xl overflow-hidden bg-white sm:h-[calc(100vh-2.5rem)] sm:max-h-[760px] sm:rounded-2xl md:grid-cols-[1.55fr_1fr]" onClick={e=>e.stopPropagation()}><button className="absolute right-3 top-3 z-20 rounded-full bg-white p-2 shadow" onClick={()=>setRoomModal(null)}><X/></button><div className="flex min-h-0 flex-col overflow-hidden bg-black"><div className="relative min-h-0 flex-1 overflow-hidden"><button type="button" onClick={() => openLightbox(roomImages.length ? roomImages : gallery, imageIndex, roomModal.name)} className="relative h-full w-full cursor-zoom-in overflow-hidden"><img aria-hidden src={roomImages[imageIndex] || gallery[0]} className="absolute inset-0 h-full w-full scale-110 object-cover opacity-30 blur-2xl"/><img src={roomImages[imageIndex] || gallery[0]} className="relative h-full w-full object-contain" alt={`${roomModal.name} ${imageIndex + 1}`}/></button>{roomImages.length > 1 && <><button onClick={()=>setImageIndex((imageIndex-1+roomImages.length)%roomImages.length)} className="absolute left-3 top-1/2 rounded-full bg-white p-2 shadow"><ChevronLeft/></button><button onClick={()=>setImageIndex((imageIndex+1)%roomImages.length)} className="absolute right-3 top-1/2 rounded-full bg-white p-2 shadow"><ChevronRight/></button></>}</div>{roomImages.length > 0 && <div className="flex h-[88px] shrink-0 gap-2 overflow-x-auto bg-black p-3">{roomImages.map((img:string,i:number)=><img key={img} onClick={()=>setImageIndex(i)} src={img} className={`h-16 w-24 shrink-0 cursor-pointer rounded object-cover ${i===imageIndex?'ring-2 ring-orange-500':'opacity-60'}`} alt={`${roomModal.name} ${i+1}`}/>)}</div>}</div><div className="flex min-h-0 flex-col overflow-y-auto p-6"><h2 className="pr-8 text-2xl font-semibold">{roomModal.name}</h2><p className="my-5 text-sm leading-6 text-gray-700">{roomModal.description}</p><div className="space-y-3 border-y py-5"><p><Maximize className="mr-2 inline text-orange-500"/> {roomModal.sizeInSqm} m²</p><p><Bed className="mr-2 inline text-orange-500"/> {roomModal.bedCount} giường</p><p><Users className="mr-2 inline text-orange-500"/> {roomModal.maxOccupancy} người</p></div><h3 className="mt-5 font-semibold">Tiện nghi</h3><div className="mt-3 space-y-2 text-sm">{(roomModal.amenities ?? []).map((a:any)=><p key={a.id}>✓ {a.name}</p>)}</div><div className="mt-auto border-t pt-5"><strong className="text-2xl text-orange-600">{Number(roomModal.pricePerNight).toLocaleString('vi-VN')}đ</strong><span> / đêm</span><button onClick={()=>goBooking(roomModal)} className="mt-4 w-full rounded-full bg-orange-600 py-3 font-semibold text-white">Chọn phòng</button></div></div></div></div>}
